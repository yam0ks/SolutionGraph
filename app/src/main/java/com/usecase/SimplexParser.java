    ...
    private void formFirstSection(){
        ...
        if(initData.getChangedRowsSign().length != 0){
            setSection("Решение базовым симплекс методом",
                            "Меняем знаки у ограничений с ≥, путём умножения на -1 Для каждого ограничения с неравенством добавляем дополнительные переменные и сотставляем начальную симплекс таблицу Начальная симплекс-таблица",
